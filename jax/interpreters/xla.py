...
def _force(x):
  if lazy.is_trivial(x._lazy_expr):
    ...
    force_fun = _lazy_force_computation(x.aval, x._device, x._lazy_expr)
    return force_fun(x)

@cache()
def _lazy_force_computation(aval, device, lexpr):
  ...
  new_lexpr = lazy.array(aval.shape)
  if lazy.is_constant(lexpr):
    force_fun = lambda _: DeviceArray(aval, device, new_lexpr, compiled.Execute([]))
  else:
    force_fun = lambda x: DeviceArray(aval, device, new_lexpr,
                                      compiled.Execute([x.device_buffer]))
  return force_fun
