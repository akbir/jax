...
from ..util import (partial, partialmethod, cache, safe_map, prod, unzip2,
                    unzip3, memoize, subvals, taggedtuple)
...
def is_lazy_constant(lexpr):
  return lexpr is not None and type(lexpr.input) is not LazyArrayVar

def is_trivial_lexpr(lexpr):
  return (type(lexpr.input) is LazyArrayVar and
...
def stage_lexpr(c, lexpr, x):
  ...
  """
  if lexpr is None or is_trivial_lexpr(lexpr):
    return x
...
def array_result_handler(device, aval):
  return partial(DeviceArray, raise_to_shaped(aval), device, lazy_array(aval.shape))
...
def device_put(x, device=None):
  return _device_put(_force(x), device)
...
def xla_primitive_callable(prim, *arg_specs, **params):
  ...
  if FLAGS.jax_log_compiles:
    logging.warning("Compiling primitive {} for args {}.".format(prim.name, arg_specs))
...
def _execute_compiled_primitive(prim, compiled, backend, tuple_args,
  ...
  input_bufs = [_device_put(x, device) for x in args
...
def _xla_callable_args_lazy(c, arg_specs, tuple_args):
  if not tuple_args:
    args = (c.ParameterWithShape(_xla_shape(aval, lexpr))
            ...
            if aval is not abstract_token and not is_lazy_constant(lexpr))
  else:
  ...
  xla_args = [stage_lexpr(c, lexpr, None if is_lazy_constant(lexpr) else next(args))
...
def _xla_shape(aval, lexpr):
  ...
  assert not is_lazy_constant(lexpr)
  if lexpr is None:
...
# Since jit forces any lazy arguments, its arg_spec function differs from that
# of op-by-op execution (the arg_spec function) in that the lazy expression
# isn't included.
def call_arg_spec(x):
  aval = abstractify(x)
  try:
...
def _xla_call_impl(fun, *args, **params):
  ...
  compiled_fun = _xla_callable(fun, device, backend, *map(call_arg_spec, args))
  try:
    return compiled_fun(*args)
...
def _execute_trivial(jaxpr, device, consts, handlers, *args):
  ...
  return [device_put_p.bind(x, device=device) if type(x) is DeviceArray
...
class DeviceArray(DeviceValue):
  ...
  __slots__ = ["_npy_value", "_lazy_expr", "_device"]
  ...
  def __init__(self, aval, device, lazy_expr, device_buffer):
    ...
    if not core.skip_checks:
      ...
      assert is_lazy_constant(lazy_expr) == (type(device_buffer) is DeviceConstant)
...
def _device_put_device_array(x, device):
  # When x has a nontrivial lazy expression, the buffer returned by this
  # function represents only the input to the lazy expression.
  if xb.get_device_backend(device).platform == x.device_buffer.platform():
   ...
   return xc.Buffer.from_pyval(x.device_buffer.to_py(), device,
                               backend=xb.get_device_backend(device))
...
def _device_put_impl(x, device=None):
  if type(x) is DeviceArray:
    # special logic to handle lazy DeviceArrays efficiently
    if is_device_constant(x):
      # return a new device constant on the given device
      return DeviceArray(x.aval, device, x._lazy_expr, DeviceConstant(device))
    else:
      # return a DeviceArray with the same lazy expression but a new underlying
      # buffer on the given device
      if device is None:
        device = xb.get_backend(None).get_default_device_assignment(1)[0]
      new_buf = _device_put_device_array(x, device)
      return DeviceArray(x.aval, device, x._lazy_expr, new_buf)
  else:
    try:
      a = abstractify(x)
    except TypeError:
...
def _force(x):
  """Force a possibly-lazy DeviceArray to be materialized."""
  if type(x) is not DeviceArray:
    return x
  if is_trivial_lexpr(x._lazy_expr):
    return x
  else:
    return apply_primitive(force_p, x, aval=x.aval)  # apply identity primitive
