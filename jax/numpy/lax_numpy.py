...
def _promote_shapes(fun_name, *args):
  ...
  if len(args) < 2:
    ...
    if not nonscalar_ranks or len(set(nonscalar_ranks)) == 1:
      ...
      return [lax.broadcast(arg, (1,) * (result_rank - len(shp)))
...
def arange(start, stop=None, step=None, dtype=None):
  ...
  if stop is None and step is None:
  ...
  elif step is None or issubdtype(_dtype(step), integer):
    dtype = dtype or result_type(*(x for x in (start, stop, step)
                                   if x is not None))
    if stop is None:
    ...
    quot, rem = divmod(stop - start, step)
    num = quot + bool(rem)
    return step * lax.iota(dtype, num) + start
  else:
    # floating point steps are hard to get right, so we build in numpy first
