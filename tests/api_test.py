...
from jax import api, lax, lax_reference, core
...
class APITest(jtu.JaxTestCase):
  ...
  def test_trivial_computations(self):
    ...
    self.assertIs(z1.device_buffer, z2.device_buffer)

    x1, x2 = np.array([1, 2]), np.array([2, 3])
    z1, z2, z3 = api.jit(lambda x, y: (y, 1, x))(x1, x2)
    self.assertIs(z1.device_buffer, x2.device_buffer)
    self.assertIs(z3.device_buffer, x1.device_buffer)
...
class LazyTest(jtu.JaxTestCase):
  ...
  def test_constant_forcing_computations_cached(self):
    ...
    with jtu.count_primitive_compiles(False) as count:
      _ = f(big_lazy_x)
